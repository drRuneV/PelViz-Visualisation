...
import ecosystem.gui.TransectInput;
...
private void drawHistogram(Graphics2D gPanel, int n, VisualDistribution vd) {
		...
		BufferedImage bs=vd.getStatistics().getHistograms()[t].drawHistogram() ;//drawHistogram(t);
		
		// Please change this to use pixel regions��� �|�|� �|�|� �|�|�
		int histogramWith= vd.getStatistics().getHistograms()[0].getInterval();
		int x= (int) (vd.getPixelRegion().x+vd.getPixelRegion().width -histogramWith -2 );
		...
		if (join && vd.getGuestList()!=null) {
			for (VisualDistribution vdg : vd.getGuestList()) {
				...
				BufferedImage biGuest=vdg.getStatistics().getHistograms()[t].drawHistogram() ;//drawHistogram(t);
	...
	private void drawGradients(Graphics2D gPanel) {
		...
		for (VisualDistribution vd : distributions) {
			if (vd.showGradient) {
				...
				// Define location of gradient in pixel space
				vd.getGradient().setLocation(new Point(x, y));
...
//		gPanel.drawString(s2, p.x, p.y+60);
	...
	private void displayName(Graphics2D g,int i) {
		...
		boolean hasTime= dist.isHasTime();
		String dateString=""; 
		
		// Finds the day month and year from the distributions date array
		if (hasTime) {
			int maxday = Math.min(count, dist.getDates().length-1) ;
			int hourSince1950= dist.getDates()[maxday];
			Clock clock= new Clock(hourSince1950,true);
			dateString =clock.dayMonthYearString() ;
		}
		String display= String.format(dist.getFullName()+" ");
		// Extra information
		if (dist.useDifference) {
			display+= String.format(" as difference "+dist.getDiffInterval()+" days ahead" );
		}

		//
		g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		// Font Size
		int fontSize= (int) Math.max(14, Math.min(26, 7*scale));
		fontSize+= (dist.getWidth()>200) ? 4: 0;
...
//		}
	...
	private void defineRegions(){
		...
		for (VisualDistribution vd : distributions) {
			...
			// Find the absolute location on the screen and define it for the distribution
...
//					fcd.setFileFilter(filter);
			...
			// Mirror everything
				...
				//		
				if (ecosystem.getSurveyLine()!=null) {
					ecosystem.getSurveyLine().setMirror(mirror);
				}
				// transect
				if (transectLine!=null) {
					transectLine.setMirror(mirror);
				}
				// SelectionMask
				distribution.getSelectedMask().setMirror(mirror);
			}

			// Open a file with the NetCDFPanel
			else if(key == KeyEvent.VK_O && ctrlDown && shiftDown){
			...
			// Show line plot along the survey line //Label7
				...
				ecosystem.thePlotAlongSurvey(distribution);
			...
			// Show surveyLine
				...
				boolean showSurveyLine= !ecosystem.getEcoVisualiser().isShowSurveyLine();
				ecosystem.getEcoVisualiser().setShowSurveyLine(showSurveyLine);
			}

			// Show transect
			else if(key == KeyEvent.VK_V  && shiftDown){
				showTransect= ! showTransect;
				transectLine.setWasCreated(showTransect);
			}
			//
			// Define the transect line by input
			else if(key == KeyEvent.VK_L && ctrlDown){
				if (transectLine!=null) {
					showTransect=true;
					Point p=  distribution.pixelAbsoluteUpperLeft();
					if (transectLine.getGraph()!=null){
						p.y+= transectLine.getGraph().getFrame().getHeight();
						//Triggers visibility of graph
						transectLine.setWasCreated(true);
					}
					TransectInput transect= new TransectInput(transectLine,p); 				
				}
			}
			...
			// Change the display mode of topography
				...
				topogradient=topography.changeMode(1);
		...
		/**
		 * Deletes the current distributionthis
		 */
...
//		super.mouseWheelMoved(e);
		...
		//
		boolean aboveLand= (controlDown && pixelInfo.isLand);
		boolean aboveGradient = (e.getX()>gradient.getLocation().x && e.getY()>gradient.getLocation().y
				 && e.getPoint().distance(gradient.getLocation()) < 90 ) ;
		...
		if( aboveGradient){
			...
			topogradient=topography.changeMode(n);
			// Redraw the topography if it is okay
			redrawTopography();			
		}
...
//			1/8= 0,125
		...
		// Change the time
		else if (!pixelInfo.outofRange  &&  (only || controlDown ) && wait) {
			int month=  (controlDown)? 30:1;
			count+= n*month;
			checkCount();
			drawDistributionValues(count);
		}
		//offset
			...
			off.x+= n;
			off.x = Math.max(1, off.x);
			off.x = Math.min(50, off.x);
			redefine();
		}
		else if(e.getY()<20){
			off.y+= n;
			off.y = Math.min(50, off.y);
			off.y = Math.max(1, off.y);
			redefine();
		}
		
		//Scale
 		else if(pixelInfo.outofRange){
			scale+= n ;
//			scale*= 1.25;
...
//			fontIndex= Math.max(0, Math.min(fontIndex, fontNames.length));
	...
	public void mouseClicked(MouseEvent me) {
		...
		boolean aboveGradient = (me.getX()>gradient.getLocation().x && me.getY()>gradient.getLocation().y
				 && me.getPoint().distance(gradient.getLocation()) < 90 ) ;
		...
		//Single click
			...
			// Interactively making survey lines //Label1          
				...
				if (ecosystem.getSurveyLine()!=null && ecosystem.getEcoVisualiser().isShowSurveyLine()) {
					ecosystem.defineSurveyLine(c);
				}
